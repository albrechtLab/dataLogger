 ...
 *     Return "stream to memory" function for super-fast logging?
 *     Add SD card logging?
 */
...
int digital = 0;     // specifies digital sensor number (<=0 for analog)

//defaults
float tempResistance = 100000.0; // Ohms at 25C
float beta = 3950.0; // check this; could also be 3950K
float scaling = 1.;
uint16_t zero = 0;
...
//unsigned int data1[MEMLEN];  // only needed for stream-to-memory
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (liveStream) {
      liveStream = false; // stop live stream if running
    ...
    if (first == 'i') {   // Change sampling interval (us)
    ...
    else if (first == 'f') {  // Flip trigger polarity (0 = LOW or 1 = HIGH)
      char* command = strtok(inputBytes, " ,") + 1;
    ...
    else if (first == 'G') {  // set live mode: no time stamp, no limit, watch for exit
      secondsToRecord = 24 * 60 * 60; // 1 day max?
      liveStream = true;
      startLogging(); 
    }

    else if (first == 'X' || first == 'x') {  // STOP/Abort
      streamRecording = false;
    }
    ...
    else if (first == 'a') {   // Analog voltage recording with scaling
      char* command = strtok(inputBytes, " ,") + 1;
      if (command != 0) zero = atol(command); else zero = 0;
      command = strtok(0, " ,");
      if (command != 0) scaling = atof(command); else scaling = 1.;      
      digital = -1;
      ...
      digital = -1;
      if (interval < 5000) interval = 5000; // min sampling interval 5ms
    }
  ...
  */
  ...
  if (streamRecording) {
    ...
    switch (digital) {
      ...
      case -1:  // read 1-2 voltage inputs and scale
        val1 = (uint16_t) ((analogRead(VIN1) - zero)*scaling);
        if (n>1) val2 = (uint16_t) ((analogRead(VIN2) - zero)*scaling);
        break;
        
      case -2:  // read analog thermistor voltage inputs
...
void showDetails() {
  Serial.print(F("FastDataLogMulti v"));
      ...
      Serial.println(F("f# sets trigger level LOW (0, default) or HIGH (1)"));
      Serial.println();
      Serial.println(F("G     GO live (no time stamp, useful for Serial Plotter)"));
      Serial.println(F("X     STOP / Abort logging session"));
      Serial.println();
      ...
      Serial.println(F("a#,#  Analog scaled input (#,# = zero,scaling)"));
      Serial.println(F("C     Digital color sensor, red and blue"));
      Serial.println(F("P     Digital voltage and current (mV, mA)"));
      Serial.println(F("t###  Analog 100k thermistor (x100 degC) ### = x10 kohms for calibration; 1000 = 100kOhm"));
