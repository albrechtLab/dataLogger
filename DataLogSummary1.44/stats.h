...
public:
  ...
  value_type add(const value_type value) {
    ...
    _lxcnt++;
    //if ((value - _avg > 0) && (_last - _avg < 0) && _lxcnt > 3) 
    //if ((value - _avg > 10) && (_last - _avg < 10) && _lxcnt > 1) 
    if ( (((value - _avg > 10) && (_last - _avg < 10)) || ((value - _avg > -10) && (_last - _avg < -10))) && _lxcnt > 1) 
    {
      _xcnt++;
      _lxcnt = 0;
    }
  ...
  count_type rise_count() const   { return _xcnt; };   //  zero if count == zero
...
protected:
  ...
  value_type _last { 0.0 };   //  NaN;
  value_type _avg { 0.0 };   //  NaN;
